...
const Announcement = ({ date, title, content, fullWidth }) => {
  // Set the container width dynamically based on fullWidth prop
  const containerClasses = fullWidth ? "w-full" : "max-w-sm";
  
  return (
    <div
      className={`${containerClasses} mx-auto bg-white rounded-lg p-4 sm:p-6 md:p-8 shadow-sm hover:bg-red-50 hover:shadow-md transition-all duration-300`}
    >
