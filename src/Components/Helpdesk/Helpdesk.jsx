...
const Helpdesk = () => {
  ...
  const handleDepartmentChange = (value) => {
    ...
    if (value === "IT" || value === "Admin") {
      ...
      setShowMessage(""); // Clear any previous message before confirmation
    } else {
      setShowConfirmation(false); // Hide confirmation if the department is changed
      setShowMessage(""); // Clear message if department is not "IT" or "Admin"
    }
  };
  ...
  const handleConfirmation = (confirm) => {
    if (confirm) {
      const url =
        selectedDepartment === "IT"
          ? "https://itsupport.logic-fruit.com/"
          : "https://itsupport.logic-fruit.com"; // URL for Admin
      window.open(url, "_blank"); // Open the URL in a new tab
  
      // Set message only after confirming and opening the page
      setShowMessage(`${selectedDepartment} page is opened in a new tab.`); 
    }
    setShowConfirmation(false); // Close the confirmation dialog after response
  };
