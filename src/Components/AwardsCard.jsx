...
const AwardCard = ({
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-96 md:h-[455px] bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      {/* Right Section (Text & User Info) */}
      <div className="w-full md:w-1/2 p-4 flex flex-col justify-between items-center md:items-start md:h-full">
        <h2 className="text-sm sm:text-2xl animate-text bg-gradient-to-r from-teal-600 via-blue-600 to-purple-600 bg-clip-text text-transparent font-black">
          <strong>{awardDetails}</strong>
        </h2>
        <h3 className="text-xs sm:text-lg mb-2 mt-2 animate-text bg-gradient-to-r from-blue-600 via-purple-500 to-teal-600 bg-clip-text text-transparent font-black">
          {message}
        </h3>
        <div className="flex flex-col md:flex-row justify-evenly items-center md:items-start mt-4">
          <div className="text-center md:text-left mr-4">
          ...
          <div className="flex flex-row justify-between">
            <div className="relative w-24 h-24 mt-4 md:mt-0">
              <img
                src={frameImage}
                alt="Frame"
                className="absolute inset-0 rounded-full object-cover border-4 border-gray-200"
                width={200}
                height={200}
              />
              <img
                src={userImage}
                alt="User"
                className="absolute inset-0 w-20 h-20 rounded-full object-cover m-2 border-2 border-gray-100"
              />
            </div>
            {/* Conditionally render the award image based on window width */}
            {windowWidth < 768 ? (
              <div className="w-24 h-24 mx-4 mt-4 md:mt-0">
                <img
                  src={image}
                  alt="Award"
                  className="rounded-full object-cover w-full h-full md:shadow-md"
                  width={150}
                  height={150}
                />
              </div>
        ...
        imagePosition === "right" && (
          <div className="w-full md:w-1/2 flex justify-center items-center">
            <img
              src={image}
              alt="Award"
              className="w-full h-full object-cover rounded-md md:shadow-md"
            />
          </div>
        )
      )}
